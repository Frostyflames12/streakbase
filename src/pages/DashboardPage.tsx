...
const FlameIcon = ({ className = "" }: { className?: string }) => (
  <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className={`w-8 h-8 ${className}`}>
    <path d="M8.5 14.5C7.5 13 7 11.5 7 9.5C7 6 9.5 3 12 2C14.5 3 17 6 17 9.5C17 11.5 16.5 13 15.5 14.5C17 15.5 18 17.5 18 19.5C18 21.5 16 23 14 23C13 23 11.5 22 12 20C12.5 18 14 16.5 14.5 15.5C13.5 15 12 14 10.5 15.5C9 17 8.5 18.5 8 20C7.5 21.5 6 23 4 23C2 23 0 21.5 0 19.5C0 17.5 1 15.5 2.5 14.5C1.5 13 1 11.5 1 9.5C1 4 6 0 12 0C18 0 23 4 23 9.5C23 11.5 22.5 13 21.5 14.5C23 15.5 24 17.5 24 19.5C24 22.5 21 24 18 24C15.5 24 14 22.5 14 21.5C14 22.5 12.5 24 10 24C7 24 4 22.5 4 19.5C4 17.5 5 15.5 6.5 14.5C5.5 13 5 11.5 5 9.5C5 6.5 7 4.5 8.5 3.5C7.5 5.5 7 7.5 7 9.5C7 11.5 7.5 13 8.5 14.5Z" fill="url(#flameGradient_dashboard)" />
    <defs>
      <linearGradient id="flameGradient_dashboard" x1="12" y1="0" x2="12" y2="24" gradientUnits="userSpaceOnUse">
      ...
      <div className={`relative max-w-md mx-auto px-5 pt-8 transition-opacity duration-700 ${mounted ? "opacity-100" : "opacity-0"}`}>
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-black tracking-tight italic">
              STREAK<span className="text-transparent bg-clip-text bg-gradient-to-r from-orange-500 to-yellow-400">BASE</span>
            </h1>
          </div>
        ...
        <div className="relative group mb-8">
          ...
          <div className="relative bg-slate-900/60 backdrop-blur-xl rounded-[1.8rem] p-8 border border-white/10 shadow-2xl overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col items-center">
              ...
              {/* Streak Count */}
              <p className="text-slate-400 text-xs font-bold uppercase tracking-[0.2em] mb-4">Current Streak</p>
                ...
                {last7Days.map((day) => {
                  ...
                  return (
                    <div key={day} className="flex flex-col items-center gap-2">
                      <span className={`text-[10px] font-bold uppercase ${isToday ? "text-orange-400" : "text-slate-600"}`}>
                        {new Date(day + "T12:00:00").toLocaleDateString("en-US", { weekday: "short" }).charAt(0)}
                      </span>
                      <div className={`
                         relative flex items-center justify-center w-4 h-4 rounded-full transition-all
                         ${isToday ? "ring-2 ring-orange-500/50 ring-offset-2 ring-offset-slate-900" : ""}
                         ${activeDates.has(day) ? "bg-orange-500 shadow-[0_0_8px_rgba(249,115,22,0.6)]" : "bg-slate-800"}
                      `}>
                        {activeDates.has(day) && <div className="absolute inset-0 bg-white/30 rounded-full animate-pulse" />}
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="mt-8 w-full bg-slate-950/40 rounded-2xl p-3 border border-white/5 flex items-center justify-between px-6">
                ...
                <div className="flex gap-2">
                  {[...Array(2)].map((_, i) => (
                    <div key={i} className={`w-8 h-8 rounded-lg flex items-center justify-center text-lg border transition-all duration-300 ${i < (profile?.freeze_tokens ?? 0) ? "bg-blue-500/20 border-blue-400/30 text-blue-200 shadow-[0_0_10px_rgba(59,130,246,0.3)]" : "bg-slate-800/50 border-slate-700 text-slate-600 opacity-50 grayscale"}`}>
                      🧊
                    </div>
                  ))}
                </div>
        ...
        {recentActivities.length > 0 && (
          <div className="mb-8 animate-in slide-in-from-bottom-3 duration-500">
            <h2 className="flex items-center gap-2 text-orange-400 text-xs font-bold uppercase tracking-[0.2em] mb-4 pl-2">
                ...
                >
                  ...
                  <div className="flex items-center justify-between p-4 relative z-10">
                    <div className="flex-1 min-w-0 mr-4">
                      <span className="text-orange-500/70 text-[10px] font-bold uppercase tracking-wider">Recent</span>
        ...
        <div className="space-y-8">
          {!categories || categories.length === 0 ? (
            <div className="bg-slate-900/50 backdrop-blur-md rounded-3xl p-10 border border-dashed border-slate-700 text-center flex flex-col items-center gap-4">
              ...
              <button onClick={() => navigate("/categories")} className="bg-white text-slate-900 px-6 py-2 rounded-full font-bold text-sm hover:scale-105 transition-transform">
                Create Category →
              </button>
            </div>
          ) : (
            categories.map((category: CategoryWithActivities) => (
              <div key={category.id} className="animate-in slide-in-from-bottom-5 duration-700 fade-in">
                <div className="flex items-center gap-3 mb-4 pl-2">
                  ...
                  <h2 className="text-slate-400 text-xs font-bold uppercase tracking-[0.2em] shadow-black drop-shadow-sm">{category.name}</h2>
                  <div className="h-[1px] flex-1 bg-gradient-to-r from-transparent via-slate-700 to-transparent opacity-50"></div>
                </div>
