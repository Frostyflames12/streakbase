                ...
                {last7Days.map((day) => {
                  ...
                  return (
                    <div key={day} className="flex flex-col items-center gap-2">
                      <span
                        className={`text-[10px] font-bold uppercase ${isToday ? "text-orange-400" : "text-slate-600"}`}
                      >
              ...
              <div className="mt-8 w-full bg-slate-950/40 rounded-2xl p-3 border border-white/5 flex items-center justify-between px-6">
                ...
                <div className="flex gap-2">
                  {[...Array(2)].map((_, i) => (
                    <div
                      key={i}
                      className={`w-8 h-8 rounded-lg flex items-center justify-center text-lg border transition-all duration-300 ${i < (profile?.freeze_tokens ?? 0) ? "bg-blue-500/20 border-blue-400/30 text-blue-200 shadow-[0_0_10px_rgba(59,130,246,0.3)]" : "bg-slate-800/50 border-slate-700 text-slate-600 opacity-50 grayscale"}`}
                    >
                      🧊
                    </div>
                  ))}
